...
for(x in 1:length(plotnames)){
  ...
  sapply(boxes@polygons,function(x) {x@area})
...
ggplot(results,aes(x=TreeCount_primary,y=TreeCount_alternative)) + geom_point() + stat_smooth(method="lm") + geom_abline() + labs(x="Tree Count (Primary Annotator)",y="Tree Count (Alternative Annotator)") + coord_equal()
ggsave("/Users/ben/Dropbox/Weecology/Benchmark/figures/treecount.png",dpi=300)

ggplot(results,aes(x=TreeHeight_primary,y=TreeHeight_alternative)) + geom_point() + stat_smooth(method="lm") + geom_abline() + labs(x="Mean Tree Height (m) - Primary Annotator",y="Mean Tree Height (m) - Alternative Annotator") + coord_equal()
